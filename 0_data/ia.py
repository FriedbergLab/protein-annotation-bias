...
def propagate_terms(terms_df, subontologies):
    """
    Propagate terms in DataFrame terms_df according to the structure in subontologies.
    If terms were already propagated with the same graph, the returned dataframe will be equivalent to the input
    
    :param terms_df: pandas DataFrame of annotated terms (column names 'EntryID', 'term' 'aspect', 'reference')
    ...
    propagated_terms = []
    if 'reference' not in terms_df.columns:
        ...
        for (protein, aspect, reference), entry_df in terms_df.groupby(['EntryID', 'aspect', 'reference']):
            protein_terms = set().union(*[list(ancestor_lookup[aspect][t])+[t] for t in set(entry_df.term.values)])

            propagated_terms += [{'EntryID': protein, 'term': t, 'aspect': aspect, 'reference': reference} for t in protein_terms]
